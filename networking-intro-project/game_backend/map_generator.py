...
class Generator():
    ...
    # put swords and coins on the road
    def gen_accessories(self):
        n = random.randint(3, 5)
        m = random.randint(1, 20)

        for i in range(n):
            x, y = random.randint(0, len(self.tiles_level[0]) - 1), random.randint(0,  len(self.tiles_level) - 1)
            while self.tiles_level[y][x] != '.':
                x, y = random.randint(0, len(self.tiles_level[0]) - 1), random.randint(0,  len(self.tiles_level) - 1)
        
            self.tiles_level[y][x] = 'T'
        
        for i in range(m):
            x, y = random.randint(0, len(self.tiles_level[0]) - 1), random.randint(0, len(self.tiles_level) - 1)
            while self.tiles_level[y][x] != '.':
                x, y = random.randint(0, len(self.tiles_level[0]) - 1), random.randint(0, len(self.tiles_level) - 1)
            self.tiles_level[y][x] = 'O'
